...
if(isset($_SESSION['id']))
{
	if($User->GetUser($_SESSION['id'], 'email') == $_SESSION['email'])
	{
		...
		if($User->GetUser($_SESSION['id'], 'password') == $_SESSION['password'])
		{
		    ...
		    if(isset($_GET['code_promo_2']))
		    {

		    	$code_promo_value = htmlentities(trim($_GET['code_promo_2']));
		    	...
		    	{
		    		if($Code_Promo->GetPromoCode($Code_Promo->GetPromoCodeIdFromSecretCode($code_promo_value), 'subscription_type') == 2)
		    		{
		    	...
		    	if($new_price != 0)
		    	{
					...
					$item = (new \PayPal\Api\Item())
						->setName('Abonnement Rock Money 1 Mois FLASH')
						->setPrice($new_price)
						->setCurrency('EUR')
						->setQuantity(1);
					$list->addItem($item);
					...
					$transaction = (new PayPal\Api\Transaction())
						->setItemList($list)
						->setDescription('Abonnement Rock Money 1 Mois FLASH')
						->setAmount($amount)
						->setCustom($_SESSION['id'].'|2|'.$code_promo_value);
					...
					$redirectUrls->setReturnUrl('http://rock-money.fr/vip.php?p=abonner_2');
					...
					if($is_valid_promo_code == true)
					{
						...
						if($Code_Promo->GetPromoCode($Code_Promo->GetPromoCodeIdFromSecretCode($code_promo_value), 'for_all') != 1)
						{
							...
							$User->SetUser($_SESSION['id'], 'subscription_type', '2');

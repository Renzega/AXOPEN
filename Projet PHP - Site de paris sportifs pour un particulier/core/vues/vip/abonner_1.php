				...
				if($real_custom_infos[1] == '1')
				{
					...
					if(strlen($real_custom_infos[2]) > 0)
					{
						if($Code_Promo->ExistsPromoSecretCode($real_custom_infos[2]))
						{
							...
							$amount = round((4.99 - (($Code_Promo->GetPromoCode($Code_Promo->GetPromoCodeIdFromSecretCode($real_custom_infos[2]), 'promo_value')*4.99)/100)), 2, PHP_ROUND_HALF_UP);
							...
							$Invoice->CreateInvoice($User->GetUser($_SESSION['id'], 'email'), 'Abonnement 1 mois STARTER', $amount, $_GET['paymentId']);
							$Paypal_Checkout->CreatePaypalCheckout($_GET['paymentId'], $_GET['token'], $_GET['PayerID']);

						} else {

							$Invoice->CreateInvoice($User->GetUser($_SESSION['id'], 'email'), 'Abonnement 1 mois STARTER', 4.99, $_GET['paymentId']);
							$Paypal_Checkout->CreatePaypalCheckout($_GET['paymentId'], $_GET['token'], $_GET['PayerID']);
							...
							$User->SetUser($real_custom_infos[0], 'subscription_type', $real_custom_infos[1]);
							$User->SetUser($real_custom_infos[0], 'subscription_date_end', date('d/m/Y', strtotime("+1 months")));
							$Invoice->CreateInvoice($User->GetUser($_SESSION['id'], 'email'), 'Abonnement 1 mois DIAMOND', 4.99, $_GET['paymentId']);
							$Paypal_Checkout->CreatePaypalCheckout($_GET['paymentId'], $_GET['token'], $_GET['PayerID']);
								...
								<div class="row">
									<div class="col-12">
										<div class="alert alert-success">
											...
											<strong>Félicitations !</strong> Vous êtes dorénavant abonné au pack Rock Money STARTER !<br />

			...
			if($result > 0) {
				...
				if($result2 == md5($password)) {
                    ...
                    if($result3 > 0) {
	                    ...
	                    $data = array("student_first_name", "student_last_name", "student_establishment");

	                 	for($i = 0; $i < sizeof($data); $i++) {
	                 		$query4 = $mysql->prepare("SELECT ".$data[$i]." FROM students WHERE student_email = :email LIMIT 1");
	                 		$query4->execute(array(":email" => $email));
	                 		$result4 = $query4->fetchColumn();
	                 		array_push($arrayJSON, $result4);
	                 	}
